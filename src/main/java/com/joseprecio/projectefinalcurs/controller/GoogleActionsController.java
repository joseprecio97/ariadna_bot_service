...
import com.joseprecio.projectefinalcurs.model.EditWelcomePhraseForm;
...
import com.joseprecio.projectefinalcurs.model.WelcomePhraseForm;
...
public class GoogleActionsController {
	 ...
	 * @throws Exception *
	*/
	private String createGaupdate(String sdkresponse, int sdkcode, ArrayList<String> validationErrors,
		 String message, Model model) throws Exception{
		...
		for(String language : GAInfo.getLanguages()) {
			if(GAInfo.getWelcomePhrases().get(language) == null) {
				GAInfo.getWelcomePhrases().put(language, new ArrayList<String>());
			}
		}
		
		//Pasamos a la vista el modelo de frase de bienvenida
		model.addAttribute("welcomephrase", new WelcomePhraseForm());
	 ...
	 */
	@PostMapping("/update")
	public String update(@ModelAttribute GAUpdateModel updateModel, Model model) throws Exception {
		
		try {
			//Actualizamos el proyecto en google actions
			GoogleActionsSDKResponse sdkResponse = googleActionsServiceImpl
														.updateGoogleActions(updateModel.getToken());
	...
	*/
	...
	/**
	 * Añade una frase de bienvenida de google actions
	 * 
	 * @param welcomePhrase
	 * @param bindingResult
	 * @param model
	 ...
	 * @throws Exception
	 */
	@PostMapping("/addwelcomephrase")
	public String addwelcomephrase(@Valid @ModelAttribute WelcomePhraseForm welcomePhrase, BindingResult bindingResult, 
			Model model) throws Exception{
		...
		//Actualizamos la información
		if(GAInfo.getWelcomePhrases().get(welcomePhrase.getLanguage()) == null) {
			GAInfo.getWelcomePhrases().put(welcomePhrase.getLanguage(), new ArrayList<String>());
		}
		
		GAInfo.getWelcomePhrases().get(welcomePhrase.getLanguage()).add(welcomePhrase.getPhrase());
		googleActionsServiceImpl.setInfo(GAInfo);
	
		//Devolvemos la vista
        return createGaupdate("", 3, null, "welcomephraseadded", model);
		
	}
	
	@GetMapping("/editwelcomephrase")
	public String editwelcomephrase(Model model) throws Exception{
		//Devolvemos la vista
        return createGaupdate("", 3, null, null, model);
	}
	
	/**
	 * Edita una frase de bienvenida de google actions
	 * 
	 * @param welcomePhrase
	 * @param bindingResult
	 * @param model
	 ...
	 * @throws Exception
	 */
	@PostMapping("/editwelcomephrase")
	public String editwelcomephrase(@Valid @ModelAttribute EditWelcomePhraseForm welcomePhrase, BindingResult bindingResult, 
			Model model) throws Exception{
		...
		//Actualizamos la información
		int oldPhraseIndex = GAInfo.getWelcomePhrases().get(welcomePhrase.getLanguage())
								.indexOf(welcomePhrase.getOldPhrase());
		GAInfo.getWelcomePhrases().get(welcomePhrase.getLanguage()).remove(oldPhraseIndex);
		GAInfo.getWelcomePhrases().get(welcomePhrase.getLanguage()).add(welcomePhrase.getPhrase());
		googleActionsServiceImpl.setInfo(GAInfo);
	
		//Devolvemos la vista
        return createGaupdate("", 3, null, "welcomephraseedited", model);
		
	}
	
	@GetMapping("/removewelcomephrase")
	public String removewelcomephrase(Model model) throws Exception{
		//Devolvemos la vista
        return createGaupdate("", 3, null, null, model);
	}
	
	/**
	 * Elimina una frase de bienvenida de google actions
	 * 
	 * @param welcomePhrase
	 * @param bindingResult
	 * @param model
	 ...
	 * @throws Exception
	 */
	@PostMapping("/removewelcomephrase")
	public String removewelcomephrase(@Valid @ModelAttribute WelcomePhraseForm welcomePhrase, BindingResult bindingResult, 
			Model model) throws Exception{
		...
		//Actualizamos la información
		int oldPhraseIndex = GAInfo.getWelcomePhrases().get(welcomePhrase.getLanguage())
								.indexOf(welcomePhrase.getPhrase());
		GAInfo.getWelcomePhrases().get(welcomePhrase.getLanguage()).remove(oldPhraseIndex);
		googleActionsServiceImpl.setInfo(GAInfo);
	
		//Devolvemos la vista
        return createGaupdate("", 3, null, "welcomephraseremoved", model);
